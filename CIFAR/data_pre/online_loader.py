...
def out_loader(val_dataset):
    ...
    if val_dataset == 'SVHN':
        ...
        out_set=SVHN('svhn/', split='test', transform=test_transform, download=False)
    elif val_dataset == 'Textures':
        if cfg.in_dataset=='Imagenet':
            transform=transform_test_largescale
        else: transform = transform_test
        out_set=torchvision.datasets.ImageFolder(root="dtd/images", transform=transform)
    elif val_dataset == 'places365':
        out_set=torchvision.datasets.ImageFolder("places365",transform=transform_test)
    elif val_dataset == 'iSUN':
        out_set=torchvision.datasets.ImageFolder("iSUN",
                                             transform=transform_test)
    elif val_dataset == 'LSUN_resize':
        out_set=torchvision.datasets.ImageFolder("LSUN_resize",
                                             transform=transform_test)
    elif val_dataset == 'LSUN_crop':
        out_set=torchvision.datasets.ImageFolder("LSUN_C", transform_test)
    # Imagenet
