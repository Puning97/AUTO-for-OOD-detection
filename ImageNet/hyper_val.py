...
if cfg.in_dataset in ['CIFAR-10','CIFAR-100']:
    ...
    model.cuda()
#mean_th,std_th=threshold_tool(model,train_loader,device)
if cfg.in_dataset=='CIFAR-10':
    mean_th=0.9991151690483093
    std_th=0.002845442621037364
elif cfg.in_dataset=='CIFAR-100':
    mean_th=0.9969596862792969
    std_th=0.011619229800999165
elif cfg.in_dataset=='Imagenet':
    mean_th=0.9369390606880188
    std_th=0.1566126048564911
if cfg.ood_score in ['msp','energy']:
    in_score,out_score=online_training(cfg.ood_score,model1,model2,val_loader,device,mean_th,std_th,mem_bank,label_mem)
    get_and_print_results(in_score, out_score)
elif cfg.ood_score =='odin':
    in_score,out_score=online_training_odin(cfg.ood_score,model1,val_loader,device,mean_th,std_th)
    get_and_print_results(in_score, out_score)
