...
class ResNet_react(AbstractResNet):

    def __init__(self, block, layers, num_classes=1000):
        super(ResNet_react, self).__init__(block, layers, num_classes)
        self.fc = nn.Linear(512 * block.expansion, num_classes)
        self._initial_weight()
...
def resnet50_react(pretrained=False, **kwargs):
    model = ResNet_react(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
    return model